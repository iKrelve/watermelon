...
- [x] 1. 创建主题 preset 基础设施
  - [x] 1.1 创建 `src/mainview/theme/presets.ts`
  ...
  - [x] 1.2 创建 `src/mainview/theme/apply.ts`
...
- [x] 2. 重构 CSS 变量体系（`index.css`）
  ...
  - [x] 2.2 重写 `:root` 和 `.dark` 色值为 Apple 风格
  ...
  - [x] 2.3 添加 Pastel 和 Midnight preset CSS
  ...
  - [x] 2.5 编写 WCAG 对比度属性测试
    - **Property 3: Text/background contrast meets WCAG AA**
    - **Validates: Requirements 2.5**

- [x] 3. Checkpoint — 确保 CSS 变量正确加载
  - 确保所有测试通过，如有问题询问用户

- [x] 4. 集成 preset 系统到状态管理
  - [x] 4.1 扩展 UIStore
...
- [x] 5. 迁移硬编码颜色到 CSS 变量
  ...
  - [x] 5.2 迁移 `TaskItemContent.tsx` 中的硬编码颜色
    - checkbox 优先级颜色：`border-red-400` → `border-priority-high` 等
    - 优先级指示器小点：`bg-red-500` → `bg-priority-high`
    - _Requirements: 4.1, 7.2_

  - [x] 5.3 迁移 `SubTaskRow.tsx` 和 `task-detail/SubTaskItem.tsx` 中的硬编码颜色
    - 同 5.2 的 checkbox 优先级颜色迁移
    - _Requirements: 4.1_

  - [x] 5.4 迁移 `TaskDetail.tsx` 中的硬编码颜色
    - 优先级选择器小点：`bg-red-500` → `bg-priority-high` 等
    - _Requirements: 4.1_

  - [x] 5.5 迁移 `TaskContextMenu.tsx` 中的硬编码颜色
    - `text-red-500` → `text-priority-high` 等
    - _Requirements: 4.1_

  - [x] 5.6 迁移 `Statistics.tsx` 中的硬编码颜色
